...
class VideoDescriptionTrain(object):
    ...
    def __init__(self, config):
        ...
        self.time_steps_decoder = config.time_steps_decoder
        self.x_data = {}
    ...
    def load_dataset(self, training_list):
        ...
        # Collect all sequences and find the maximum sequence length
        all_sequences = [cap[0] for cap in training_list]
        max_sequence_length = max(len(seq.split()) for seq in all_sequences)

        for cap in training_list:
            caption = cap[0]
            videoId.append(cap[1])
            videoSeq.append(caption)

        train_sequences = self.tokenizer.texts_to_sequences(videoSeq)
        train_sequences = pad_sequences(train_sequences, padding='post', truncating='post', maxlen=max_sequence_length)
